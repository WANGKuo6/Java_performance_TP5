...
public class CPainting extends Canvas implements MouseListener {
  ...
  public void setMatrice(int nbreMension,int x, int y, Color c, float[][] matrice) {
    ...
    float R, G, B;
    Color lColor;
      for (i = 0; i < nbreMension; i++) {
          for (j = 0; j < nbreMension; j++) {
            R = G = B = 0f;

            for (k = 0; k < nbreMension; k++) {
              for (l = 0; l < nbreMension; l++) {
                m = (x + i + k - (nbreMension-1) + mDimension.width) % mDimension.width;
                n = (y + j + l - (nbreMension-1) + mDimension.height) % mDimension.height;
      ...
      switch (pTaille) {
        ...
        case 1:
          // produit de convolution discrete sur 9 cases
        	setMatrice(3, x, y, c, mMatriceConv9);
          break;
        case 2:
          // produit de convolution discrete sur 25 cases
        	setMatrice(5, x, y, c, mMatriceConv25);
          break;
        case 3:
          // produit de convolution discrete sur 49 cases
        	setMatrice(7, x, y, c, mMatriceConv49);
          break;
      }// end switch
    }
  }
